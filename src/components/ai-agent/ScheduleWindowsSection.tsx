...
export default function ScheduleWindowsSection({ token, idAgente, canEdit }: ScheduleWindowsSectionProps) {
  ...
  const fetchWindows = async () => {
    setLoading(true);
    try {
      const res = await fetch(`https://n8n.lumendigital.com.br/webhook/prospecta/multiagente/agendamento/horario/get?id_agente=${idAgente}` , {
        headers: { token },
      });
  ...
  const handleSave = async () => {
    ...
    try {
      const url = Id
        ? 'https://n8n.lumendigital.com.br/webhook/prospecta/multiagente/agendamento/horario/update'
        : 'https://n8n.lumendigital.com.br/webhook/prospecta/multiagente/agendamento/horario/create';
      const method = Id ? 'PUT' : 'POST';
      const body = Id
        ? { Id, dia_semana: day, horarioInicio, horarioFinal, id_agente: idAgente }
        : { dia_semana: day, horarioInicio, horarioFinal, id_agente: idAgente };
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const res = await fetch(
        `https://n8n.lumendigital.com.br/webhook/prospecta/multiagente/agendamento/horario/delete?id=${id}&id_agente=${idAgente}`,
        {
          method: 'DELETE',
          headers: { token },
        }
  ...
  return (
    <section className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mt-6">
      <h2 className="text-xl font-semibold text-gray-900 mb-6">Horários Disponíveis</h2>
      {days.map((day) => (
