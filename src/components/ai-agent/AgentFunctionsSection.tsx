...
const AgentFunctionsSection: React.FC<AgentFunctionsSectionProps> = ({ token, canEdit }) => {
  ...
  const [newFunction, setNewFunction] = useState({
    ...
    descricao: '',
    isAtivo: true,
    tipo: 'NOTIFICACAO' as AgentFunctionType,
    mensagem: ''
  });
  ...
  const handleDeleteFunction = async (id: number) => {
    ...
    setSaving(true);
    try {
      const res = await fetch(`https://n8n.lumendigital.com.br/webhook/prospecta/funcao/delete?id=${id}`, {
        method: 'DELETE',
        headers: { token }
      });
  ...
  const handleUpdateAttribute = async (func: AgentFunction, attr: AgentFunctionAttribute) => {
    if (func.tipo === 'NOTIFICACAO') {
      ...
      setSaving(true);
      try {
        const res = await fetch('https://n8n.lumendigital.com.br/webhook/prospecta/funcao/atributo/update', {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
            token
