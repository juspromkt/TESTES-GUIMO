...
export default function OperatingHoursSection({ token, idAgente, canEdit }: OperatingHoursSectionProps) {
  ...
  useEffect(() => {
    const fetchSettings = async () => {
      setLoading(true);
      try {
        const response = await fetch(`https://n8n.lumendigital.com.br/webhook/prospecta/multiagente/funcionamento?id_agente=${idAgente}`, {
          headers: { token }
        });
        if (!response.ok) {
  ...
  const handleSave = async () => {
    ...
    try {
      const response = await fetch('https://n8n.lumendigital.com.br/webhook/prospecta/multiagente/funcionamento/create', {
        method: 'POST',
        headers: {
