...
export default function TriggerSection({ token, idAgente, canEdit }: TriggerSectionProps) {
  ...
  const fetchTriggerConfig = async () => {
    try {
      const response = await fetch(`https://n8n.lumendigital.com.br/webhook/prospecta/multiagente/gatilho/get?id_agente=${idAgente}`, {
        headers: { token }
      });
      const data = await response.json();
      if (Array.isArray(data) && data.length > 0) {
        setIsActive(Boolean(data[0].isAtivo));
  ...
  const handleToggleStatus = async () => {
    ...
    setError('');
    try {
      const response = await fetch('https://n8n.lumendigital.com.br/webhook/prospecta/multiagente/gatilho/toggle', {
        method: 'POST',
        headers: {
      ...
      const data = await response.json();
      setIsActive(Boolean(data.isAtivo));
  ...
  const handleSave = async () => {
    ...
    setError('');
    try {
      const response = await fetch('https://n8n.lumendigital.com.br/webhook/prospecta/multiagente/gatilho/create', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          token
        },
        body: JSON.stringify({ isAtivo: isActive, gatilho: trigger, id_agente: idAgente })
      });
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <label className="relative inline-flex items-center cursor-pointer">
          <input
            ...
            disabled={!canEdit || togglingStatus}
          />
      ...
      <div className="space-y-4">
        <div>
          ...
          <textarea
            ...
            disabled={!canEdit}
            className="w-full px-4 py-3 text-gray-900 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500"
            rows={3}
